...
`config/music.yaml` 负责控制博客侧边栏内置的精美音乐播放器。
...
| **本地独立模式** | `"local"` | `data/music.ts` | 零外部网络依赖，首屏毫秒级就绪，断网离线也能正常播放 |
| **自定义模式** | `"custom"` | YAML 中的 `tracks` 列表 | 适合直接在配置文件中编写几首外链歌曲 |
| **云端歌单模式** | `"meting"` | 网易云 / QQ 音乐公开歌单 | 仅拉取远端平台歌单，曲库海量且封面自动解析 |

> 配图说明：
> - 建议截图内容：侧栏音乐播放器展开状态，展示正在旋转的专辑封面、歌曲名、进度条与下拉播放列表
> - 对应保存路径：`docs/images/02-config/07-music-player-preview.png`

![侧栏音乐播放器展开与播放列表界面](../images/02-config/07-music-player-preview.png)
...
# 默认播放循环模式："sequence" 顺序播放 | "loop" 单曲循环 | "random" 随机播放
