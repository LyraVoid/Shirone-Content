...
遵循 [llms.txt 官方规范](https://llmstxt.org/) 与「零额外负担」原则，Shirone 主题在纯服务端静态生成 `/llms.txt`（精简索引）与 `/llms-full.txt`（全量正文汇编），为 ChatGPT、Claude、Perplexity、Cursor 等大模型提供结构化的 Markdown 索引，且对前台读者的浏览体验与客户端 JS 体积 0 影响。
...
siteSummary: "这是一个专注于现代前端架构与二次元视觉设计的个人博客。"
  ...
  - title: "Open Source Projects"
    ...
    items:
      - title: "Shirone Theme"
        ...
        description: "A Material 3 Expressive anime blog theme for Astro."
```
...
- **场景 B：完全关闭 AI 检索端点**
  在 `config/llms.yaml` 中设置 `enable: false`，访问对应端点返回 404，不产生任何构建文件。
- **场景 C：文章量极大时仅生成精简目录**
