...
本篇文档汇总了在内容分离架构使用与自动化部署过程中可能遇到的常见疑问与排错方法。
...
### 产生原因
在创建项目时误选了 Worker 模式或在部署命令中填写了 `npx wrangler deploy`，导致平台自动安装了 `@astrojs/cloudflare` 适配器，将静态渲染强行切换到了非 Node.js 的边缘运行时。

### 解决方法
1. Shirone 是纯静态网站生成，无需使用 Worker 服务端适配器；
2. 登录 Cloudflare 控制台，进入 **Compute (Workers & Pages)** -> **Create** -> 在页面底部选择 **Pages** 模式创建项目；
